...
homepage or via the public GitHub account for the project.
				...
				-----------------------
Min supports three different input mediums and they are: drawing, images, and typing
...
recognition inserted almost immediately to the canvas. For typed expressions, the expressions 
...
- 	The event system makes use of jQuery events and jQuery can be found at: http://jquery.com/
				...
				--------------------------------
Min can categorized based on these categories:
	1)	Editor Modes - Min's mode switching mechanism.
	2)	Event System - Min's response to user actions.
	3)	Segments Types - Currently allows segment types.
	4)	Classification / Recognition - Classification of segment types.
	5)	Canvas object layout - Rendering of bounding boxes on segments.
	6)	Correction menu - Allows user to correct or change recognition.
	7)	Utilities - Includes data structures like Tuples used in Min.
	8)	Actions - Defines all possible actions users can perform in Min.
...
upon keyboard stroke detection and typing has stopped, the object's finishEntry method is 
called. Pen strokes will activate Editor.DrawMode which records the position of the strokes 
...
Responses from the classification servers come back as XML and then used by Min to
...
Here is an example for each segment type's XML that is to be sent for classification:
	Pen strokes: The XML includes the scale and translation for the stroke. A list of the
				...
				<Segment type="pen_stroke" instanceID="0" scale="1,1" translation="401,232"
				points="0,7|0,6|8,0|9,0|...|32,29|32,30"/>
...
TeX_Input segment type doesn't require an SVG to be inserted into its OCR overlay. For 
...
This operation is done by the RenderManager in the render_set_field() function of RenderManager.js.
