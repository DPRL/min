...
\begin{figure}[h]
    \begin{center}
        \includegraphics[width=0.7\textwidth]{min_process.png}
    \end{center}
...
\begin{description}
	\item[Editor.StrokeSelectMode and Editor.RectSelectMode] These files make two calls 
		each to the CollisionManager. First, onMouseDown to determine if the user clicked 
		on a segment and if so, add it to the selected_segments array. Second, calls the 
		CollisionManager during the movement phase. Each call to the CollisionManager 
		just returns an array with the collided segments. To eliminate these calls, we tried binding 
		mouseover events to the segments but noticed that in StrokeSelectMode, the mouseover events 
		don't get triggered because the user's mouse was too fast. This approach seemed 
		like it would work for RectSelectMode because we could just add segments in the 
		selected_segments array by binding a mouseover event on the bounding box but that 
		didn't work. This is because of how we are currently using the SVG element that 
		pertains to each stroke. Currently, the SVG element for each stroke covers the entire canvas 
		so on mouse over the last segment's SVG element is the only one being triggered.
		The last stroke's SVG covers the entire canvas and previous strokes are below it.
		It's just like a stack of SVG's.
\end{description}

We agreed that moving functions specific to each Editor Mode file may help keep code clean
...
canvas (\begin{verbatim} 100% for the width and height \end{verbatim}). This solution was 
...
In all, the CollisionManager file will be there for now. 
