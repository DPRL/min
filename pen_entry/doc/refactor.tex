...
The segment results contains the coordinates of where each recognized segment on the canvas should be placed.
This information is extracted and used to transform each segment after the coordinate and scale of each 
segment is calculated and effected. We make use of a 'class' called TransformSegments and it's 
...
the SVG use tag MathJax uses to reference the \verb+MathJax_SVG_glyphs that holds the paths for each element.
...
\begin{verbatim}
<use xmlns="http://www.w3.org/2000/svg" xlink:href="#MJMAIN-32" xmlns="http://www.w3.org/1999/xlink"/>
\end{verbatim}
